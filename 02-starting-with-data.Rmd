...
>  (`20150514_KHW_Portal_joined.csv`) as an example
...
single animal. Yesterday we cleaned the data up into a nice format and joined it
with information about the survey plots and species information.

```{r, echo=FALSE, include = TRUE, purl=FALSE}
surveys <- read.csv('20150514_KHW_Portal_joined.csv')
head(surveys)
```

To load our survey data, we need to locate the `20150514_KHW_Portal_joined.csv` file. We will use
`read.csv()` to load into memory (as a `data.frame`) the content of the CSV
file.

```{r, eval=FALSE,  purl=FALSE}
surveys <- read.csv('data/20150514_KHW_Portal_joined.csv')
```
