---
layout: topic
title: Analyzing data with dplyr
---
...
group. For example, to view mean `weight` by sex:
...
Looks like most of these species were never weighed. We could then discard rows where `mean_weight == NaN` (`NaN` refers to "Not a Number") using `filter()`:
