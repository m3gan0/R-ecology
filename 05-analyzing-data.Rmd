...
surveys <- read.csv(file="20150514_KHW_Portal_joined.csv")
```
...
```{r, purl=FALSE}
weight_noNA <- na.omit(surveys$weight)
```
...
> ### Challenge {.challenge}
> 
> To determine the number of elements found in a vector, we can use use the
> function `length()` (e.g., `length(surveys$weight)`). Using `length()` and
> `na.omit()`, how many animals have not had their weights recorded?
...
## 3. Bonus question: what is the standard error for the weight? (hints: there is
##    no built-in function to compute standard errors, and the function for the
##    square root is `sqrt()`).
```

**Stop here and cover analysis with dplyr and plotting with ggplot2**
...
```{r, purl=FALSE}
tapply(surveys_complete$weight, surveys_complete$species_id, mean)
```
...
species_mean <- tapply(surveys_complete$weight, surveys_complete$species_id, mean)
```
...
```{r, purl=FALSE}
species_max <- tapply(surveys_complete$weight, surveys_complete$species_id, max)
species_min <- tapply(surveys_complete$weight, surveys_complete$species_id, min)
species_sd <- tapply(surveys_complete$weight, surveys_complete$species_id, sd)
nlevels(surveys_complete$species_id) # or length(species_mean)
surveys_summary <- data.frame(species_id=levels(surveys_complete$species_id),
                              mean_weight=species_mean,
                              sd_weight=species_sd,
                              min_weight=species_min,
                              max_weight=species_max)
```
...
```{r, purl=FALSE}
barplot(surveys_summary$mean_weight)
```

The axis labels are too big though, so you can't see them all. Let's change that.

```{r, purl=FALSE}
barplot(surveys_summary$mean_weight, cex.names=0.4)
```
