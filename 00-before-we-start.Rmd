...
* Collaborate
* Redo and easily modify analyses, figure generation, etc.
...
command. You can type directly into the console. For example, R can do
arithmetic:
...
The commands you type in the console will be forgotten when you close the program.
...
Functions in R let you perform various operations, such as finding the mean of a
set of numbers, for example. Functions have a name followed by a set of
parentheses. A function usually gets one or more inputs called *arguments*,
which get typed within the parentheses. Functions often (but not always) return
a *value*. A typical example would be the function `sqrt()`. The input (the
argument) must be a number, and the return value (in fact, the output) is the
square root of that number. Executing a function ('running it') is called
*calling* the function. An example of a function call is:

`r sqrt(16)`

Here, the value of `16` is given to the `sqrt()` function, the `sqrt()` function
calculates the square root, and returns the value. This function is very simple,
because it takes just one argument. It is useful to specify the arguments by
their name, especially if the function can takes several arguments. For example,
the `rep()` function repeats an input a specified number of times:
...
To see these default values and get other information about a function, see the
help for this function (e.g., `?rep`).
...
(Maybe return to this at a later point in the lesson)
